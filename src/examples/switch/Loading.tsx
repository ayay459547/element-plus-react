...
const SwitchLoading: React.FC = () => {
  const [value1, setValue1] = useState<ElSwitchProps['value']>(true)
  const [value2, setValue2] = useState<ElSwitchProps['value']>(false)

  return (
    <>
      <ElSwitch value={value1} onChange={setValue1} loading={true} />
      <ElSwitch value={value2} onChange={setValue2} className="ml-2" loading={true} />
    </>
  )
}
